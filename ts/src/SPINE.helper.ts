...
import * as SPINE from "./EEBUS/SPINE.types";

export class SPINEHelper {
    constructor(private readonly spine: SPINEClient) { }

    public async readAndSave<TResponse>(entity: number[], feature: number, readFunction: string) {
        const result = await this.spine.readFunction<TResponse>(entity, feature, readFunction);
    ...
    public subscribe(entity: number[], feature: number) {
        this.spine.subscribe(entity, feature, p => {
